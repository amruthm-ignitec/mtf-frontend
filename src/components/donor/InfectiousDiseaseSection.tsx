...
export default function InfectiousDiseaseSection({ data, serologyResults, cultureResults, criticalLabValues, documents = [], onCitationClick }: InfectiousDiseaseSectionProps) {
  ...
  // Handle both formats:
  // 1. Test_Results array (new format from component extraction)
  // 2. Test_1, Test_2, etc. (legacy format)
  let testResults: any[] = [];
  
  // Check for Test_Results array format first
  if (extractedData['Test_Results'] && Array.isArray(extractedData['Test_Results'])) {
    testResults = (extractedData['Test_Results'] as any[]).map((testData) => {
      return {
        testName: testData['Test_Name'] || testData['Test Name'] || 'Unknown Test',
    ...
    // Fall back to legacy format: Test_1, Test_2, etc.
    testResults = Object.entries(extractedData)
      .filter(([key, value]) => {
